...
- `.ai/process/` — the four process chapters (`two-role.md`, `pipeline.md`, `autopilot.md`, `free.md`): each profile's roles-and-phases contract — `free.md` the one that binds none — shipped **verbatim** (no markers, no project facts, never edited in a target)
...
- `bin/autopilot-driver.sh` — the autopilot profile's driver (ADR-0008): deterministic dispatch of flight phases 2–9, preflight, the write-set wall (an edge per dispatched phase, measured on each phase's git diff; git's index-masking bits refused fail-closed; test paths from the target's versioned `.ai/wall.env`; guarantee declaredly against the out-of-role phase, not the adversarial actor — ADR-0008's declared boundary), the phase-8 acceptance stamp (an empty commit with the reserved `Autopilot-Green` trailer — phase 9's entry proof, walked from `HEAD` through artifact-only commits), counters, git/PR ribbon; ships inside the plugin, launched by `/fly`, holds no model names (the machine binding comes from the target's `.ai/autopilot/models.env`)
...
- `tests/verify-kit/run.sh` — `verify-kit`'s behavior suite: disposable target-shaped fixtures (live docs, manifest, the six installed files copied from the checkout), one assertion per PASS/FAIL line by check name — the `free-shape` check and the six-file install integrity are pinned here, plus the kit-mode chapter invariants on `free.md`. Same rule as the driver suite: tests the kit, never installed
...
One kit, four **profiles**, chosen per task (ADR-0001, ADR-0008, ADR-0009). The three method profiles share the hard wall — whoever writes tests never writes application code, and the implementer never touches tests — and differ in how the design side is staffed and who supervises the handoffs; the fourth binds none of it:
...
- **free** — the deliberate exit (ADR-0009): no role, no phase, no wall, no TDD order, no roster model check; the shell below line 1 of `CLAUDE.md` and the facts contract still bind, the other three chapters are reference material applied only on explicit request. Entered and left only through `/switch-profile` (never offered at init); under it `AGENTS.md` is a symlink to `CLAUDE.md` and the implementer contract is parked, versioned, at `.ai/AGENTS.parked.md` (both moves fail closed; `verify-kit` `free-shape`). `.ai/plans/` is inert in full and never written. The chapter uses no role name and proposes no convention for the method under test.

Three design principles run through every file:

- **Shell + chapter.** A target's `CLAUDE.md` is a shell: **line 1** imports the active chapter (`@.ai/process/<profile>.md`), and everything below — shared process sections + project overlay — is identical under every profile. Switching rewrites the import line and `.ai/kit.json`, nothing else (`/switch-profile`) — except with `free` at either end, where the implementer contract also moves between `AGENTS.md` and `.ai/AGENTS.parked.md`.
...
1. **The profile triad.** `.ai/kit.json` `profile` ↔ `CLAUDE.md` line-1 import ↔ chapter filename in `.ai/process/`. Set by `/init-architecture` (Phase 3), changed only by `/switch-profile`; nothing else may restate the active profile. `verify-kit` target mode checks it (`kit-manifest`). The manifest's second field, `kitVersion`, is the install stamp: written from `.claude-plugin/plugin.json` `version` — the only place a kit version is stated — at init, restamped by `/update-kit`, the procedure that realigns the install; `verify-kit -p <plugin-root>` is the check that compares the two, plus the six installed kit files byte-for-byte (`kit-version` / `install-files`, target mode — ADR-0005). The triad is the whole of the switch under the three method profiles; with `free` at either end the switch also moves the implementer contract (`AGENTS.md` ↔ `.ai/AGENTS.parked.md`, symlink `AGENTS.md → CLAUDE.md` while `free` is active) — `free-shape` checks that shape under every profile, in target mode.
2. **Detection strings.** `/init-architecture` Phase 0 routes on the absence of `.ai/kit.json` plus `You are the **architect**` in the live `CLAUDE.md` (→ legacy-kit appendix) or pipeline-role text (→ pre-profile re-init). And the pipeline chapter must contain `Test-Writer` and must **never** use the bare word "architect" — that word belongs to `two-role.md` and the roster only; the autopilot chapter must contain `Test Writer` (space — its own role) and never `Test-Writer` (hyphen — pipeline's) nor bare "architect"; the free chapter binds no role, so it must contain **none** of them — neither `Test-Writer` nor `Test Writer` nor bare "architect" (`verify-kit` `chapters` check; in target mode the architect grep runs only on pre-profile installs, where the union roster doesn't yet legitimize it). One more word to keep out of `free.md`: the Implementer's default roster row is a harness name, and the kit-mode `model-roster` scan reads every `.md` in the repo — name "another agent CLI", never the product.
...
7. **Layer vocabulary** Model / View / Controller (orchestration) / Service / Client — fixed, verbatim, across all three doc templates, both plan templates, and the three method chapters (`free.md` states no layer rule of its own — it defers to the shell below line 1) (Contract §4 of `PROJECT_ARCHITECTURE.template.md`).
...
```bash
sh tests/verify-kit/run.sh      # verify-kit's behavior suite: target fixtures (free-shape, install
                                # integrity at six files) + the kit-mode invariants on free.md (~5 s)
