...
make_target() {
  ...
  for c in two-role pipeline autopilot free; do
    [ -f "$KIT/.ai/process/$c.md" ] && cp "$KIT/.ai/process/$c.md" "$T/.ai/process/"
  done
...
line()    { grep -Eq "^$1 +$2( |$)" "$OUT"; }          # line PASS free-shape
...
chk 'free well-formed: free-shape PASS' line PASS free-shape
chk 'free well-formed: kit-manifest PASS' line PASS kit-manifest
chk 'free well-formed: live-docs PASS through the symlink' line PASS live-docs

# ------------------------------------------------- free, absolute symlink target --
make_target free-abs free; park_agents "$T" "$T/CLAUDE.md"
run_vk "$T"
chk 'free, absolute symlink to CLAUDE.md: free-shape PASS' line PASS free-shape
...
chk 'free, AGENTS.md regular file: free-shape FAIL' line FAIL free-shape
...
run_vk "$T"
chk 'free, no parked file: free-shape FAIL' line FAIL free-shape
...
run_vk "$T"
chk 'free, symlink elsewhere: free-shape FAIL' line FAIL free-shape
...
run_vk "$T"
chk 'free, dangling symlink: free-shape FAIL' line FAIL free-shape
...
cp "$T/AGENTS.md" "$T/.ai/AGENTS.parked.md"
run_vk "$T"
chk 'pipeline, leftover parked file: free-shape FAIL' line FAIL free-shape
...
rm "$T/AGENTS.md"; ln -s CLAUDE.md "$T/AGENTS.md"
run_vk "$T"
chk 'pipeline, symlinked AGENTS.md: free-shape FAIL' line FAIL free-shape
...
chk 'pipeline clean: free-shape PASS' line PASS free-shape
...
run_vk "$T"
chk 'two-role clean: free-shape PASS' line PASS free-shape
make_target two-role-parked two-role
cp "$T/AGENTS.md" "$T/.ai/AGENTS.parked.md"
run_vk "$T"
chk 'two-role, leftover parked file: free-shape FAIL' line FAIL free-shape
...
run_vk "$T" -p "$KIT"
chk 'six identical files: install-files PASS' line PASS install-files
chk 'six identical files: the count says 6' grep -Eq '^PASS +install-files +all 6 ' "$OUT"
chk 'six identical files: kit-version PASS' line PASS kit-version

make_target install-nofree pipeline
rm "$T/.ai/process/free.md"
run_vk "$T" -p "$KIT"
chk 'free.md missing: install-files FAIL' line FAIL install-files
...
run_vk "$T" -p "$KIT"
chk 'free.md edited at the same version: install-files FAIL' line FAIL install-files
...
run_vk "$K"
chk 'free.md removed: chapters FAIL' line FAIL chapters
