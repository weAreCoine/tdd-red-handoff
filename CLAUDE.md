...
- `bin/autopilot-driver.sh` — the autopilot profile's driver (ADR-0008): deterministic dispatch of flight phases 2–9, preflight, the write-set wall (three edges measured on each phase's git diff; test paths from the target's versioned `.ai/wall.env`, fail-closed), counters, git/PR ribbon; ships inside the plugin, launched by `/fly`, holds no model names (the machine binding comes from the target's `.ai/autopilot/models.env`)
