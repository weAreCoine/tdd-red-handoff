...
| `.ai/wall.env` | The wall's machine-readable test paths (autopilot): what the driver enforces every phase's write-set against — **versioned**, same wall on every clone | **Facts** — one `AP_WALL_TESTS` line, written by `/fly` or `/init-architecture` |
...
**Prerequisites:** [Claude Code](https://www.anthropic.com/claude-code), and a project with a dependency manifest (`package.json`, `composer.json`, `pyproject.toml`, `go.mod`, …). The **autopilot** profile additionally requires: the Codex CLI and an authenticated GitHub CLI (`gh auth status`), an `origin` remote with a `develop` branch (the fixed base branch — ADR-0008), and three interview skills — `grilling`, `grill-with-docs`, `domain-modeling`, public source [mattpocock/skills](https://github.com/mattpocock/skills) — as hard dependencies: `/fly` checks all of this and refuses to open a flight otherwise.
