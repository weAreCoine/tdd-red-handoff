...
One kit, two **profiles**, chosen **per task, not once per project**:
...
The kit's fix is structural, and it holds under both profiles: the design side writes failing tests and hands the implementer a plan; the implementer writes the minimum code to turn them green and **cannot modify the tests**. That guarantee doesn't depend on model diversity to pay off — it's the wall, not the tiers, that keeps the spec fixed.

What the profiles change is how the design side is staffed:

| | **two-role** | **pipeline** |
|---|---|---|
| Design side | **Architect** — one role does design, tests, plan, review | **Designer** (spec) → **Test-Writer** (transcription) → **Verifier** (gate, plan, review) |
| Implementer | external code-gen agent (`AGENTS.md`) | same |
| Handoff artifacts | `{feature}.md` (implementation plan) | `{feature}.testplan.md` (test-case inventory) + `{feature}.md` |
| Sessions per feature | fewest | five fresh sessions |
| Worth it when | small or low-stakes changes | code that holds real data |

The selection criterion is the **stakes of the work** versus the friction the pipeline imposes: five fresh sessions and two artifacts per feature earn their keep on code that matters, and cost more than they return on a quick fix. That judgement changes task by task — so switching is a routine, bidirectional operation (`/switch-profile`), not a migration.
...
| `.ai/process/two-role.md`, `.ai/process/pipeline.md` | The roles-and-phases contract of each profile — both installed, so switching is offline | **Process** — ships verbatim, never edited |
...
| `.ai/templates/test_plan_template.md` | Designer → Test-Writer handoff: the test-case inventory (pipeline only) | **Process** |
| `.ai/templates/plan_template.md` | Design side → Implementer handoff: the implementation plan (both profiles) | **Process** |
...
The `CLAUDE.md` shell is what makes switching cheap: its **line 1** is `@.ai/process/two-role.md` or `@.ai/process/pipeline.md`, and everything below it — shared process sections, project overlay — is identical under both profiles. The import line, the `profile` field of `.ai/kit.json`, and the chapter filename must always agree (the **profile triad**); `/switch-profile` owns all three, and nothing else may restate the active profile.
...
Switching profiles never destroys the other profile's artifacts: `{feature}.md` plans are readable under both, and `{feature}.testplan.md` files simply go **inert** under two-role — never rewritten, moved, or deleted — and come back to life on the way back to pipeline.
...
1. **Profile** — which process contract the first task runs on: **two-role** or **pipeline**. Both chapters are installed either way; `/switch-profile` changes this per task later.
